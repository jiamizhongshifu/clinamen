...
(() => {
    ...
    const fragmentSource = `
      ...
      void main() {
        ...
        for (int i = 0; i < 48; i += 1) {
          ...
          float nd = dot(nq, nq);
          float reflectionBody = 1.0 - smoothstep(0.04, 1.06, nd);
          float reflectionEdge = 1.0 - smoothstep(0.72, 1.08, nd);
          float verticalFade = 1.0 - smoothstep(0.92, 1.12, abs(nq.y));
          float surfaceBreak = 0.80 + 0.20 * noise((warpedUv + nq * 0.035) * vec2(28.0, 18.0) + grad * 18.0 + uTime * 0.018);
          contactReflection += (reflectionBody * 0.70 + reflectionEdge * 0.30) * verticalFade * surfaceBreak * uSurfaceOpacity[i];
  ...
  function drawWaterGL(now) {
    ...
    for (const b of shadowBowls) {
      ...
      const farT = 1 - depthT;
      const base = count * 4;
      const bottomOffsetY = b.r * (0.98 + farT * 0.66);
      ...
      waterSurfaceShadowAlpha[count] = 0.30 * visibleFade;
      waterShadowData[base] = screenX / width;
      waterShadowData[base + 1] = (screenY + bottomOffsetY) / height;
      waterShadowData[base + 2] = (b.r * (1.04 + farT * 0.12)) / width;
      waterShadowData[base + 3] = (b.r * (0.94 + nearT * 0.12 - farT * 0.08)) / height;
      waterShadowAlpha[count] = (0.38 + nearT * 0.14 + farT * 0.16) * visibleFade;
      count += 1;
    }
    waterGL.useProgram(waterProgram);
    waterGL.activeTexture(waterGL.TEXTURE1);
    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float downwardOuter = 1.0 - smoothstep(-0.10, 0.30, localN.y);
          float outerWall = sideFacing * downwardOuter * smoothstep(0.28, 0.62, radialOut);
