...
(() => {
    ...
    const fragmentSource = `
      ...
      void main() {
        ...
        for (int i = 0; i < 48; i += 1) {
          ...
          float nd = dot(nq, nq);
          float reflectionShape = 1.0 - smoothstep(0.18, 1.04, nd);
          reflectionShape *= 1.0 - smoothstep(0.86, 1.10, abs(nq.y));
  ...
  function drawWaterGL(now) {
    ...
    for (const b of shadowBowls) {
      ...
      const reflectionOffsetY = b.r * (0.52 + nearT * 0.28 + farT * 0.10);
      ...
      waterSurfaceShadowAlpha[count] = 0.42 * visibleFade * (0.52 + nearT * 0.48);
    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float sideBlueWrap = sideFacing * (0.44 + 0.42 * smoothstep(0.0, 0.52, -n.y + 0.22));
          float lowBody = smoothstep(0.02, 0.12, h) * (1.0 - smoothstep(0.72, 0.96, h));
          float blueBounce = clamp((bottomLambert * 0.66 + sideBlueWrap) * lowBody * (0.66 + submerged * 0.34), 0.0, 1.0);
