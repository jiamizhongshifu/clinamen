...
(() => {
    ...
    const fragmentSource = `
      ...
      void main() {
        ...
        for (int i = 0; i < 48; i += 1) {
          ...
          float mirroredY = nq.y + 0.10;
          float silhouetteWidth = mix(0.70, 1.08, smoothstep(-0.70, 0.78, mirroredY));
          float bowlMirrorD = (nq.x / max(0.08, silhouetteWidth)) * (nq.x / max(0.08, silhouetteWidth)) + mirroredY * mirroredY * 0.88;
          float mirrorBody = 1.0 - smoothstep(0.84, 1.08, bowlMirrorD);
          float mirrorRim = 1.0 - smoothstep(0.030, 0.105, abs((nq.x * nq.x * 0.82 + (nq.y + 0.64) * (nq.y + 0.64) * 8.0) - 0.58));
          float mirrorLowerLip = (1.0 - smoothstep(0.022, 0.095, abs((nq.x * nq.x * 0.66 + (nq.y - 0.70) * (nq.y - 0.70) * 5.2) - 0.62))) * smoothstep(0.18, 0.82, nq.y);
          float reflectionShape = max(mirrorBody * 0.68, max(mirrorRim * 0.42, mirrorLowerLip * 0.34));
          float surfaceBreak = 0.78 + 0.22 * noise((warpedUv + nq * 0.035) * vec2(28.0, 18.0) + grad * 18.0 + uTime * 0.018);
  ...
  function drawWaterGL(now) {
    ...
    for (const b of shadowBowls) {
      ...
      const reflectionOffsetY = b.r * (0.74 + nearT * 0.48 + farT * 0.28);
      waterSurfaceShadowData[base] = screenX / width;
      waterSurfaceShadowData[base + 1] = (screenY + reflectionOffsetY) / height;
      waterSurfaceShadowData[base + 2] = (b.r * (1.02 + nearT * 0.14 + farT * 0.04)) / width;
      waterSurfaceShadowData[base + 3] = (b.r * (0.64 + nearT * 0.22 - farT * 0.16)) / height;
      const visibleFade = smoothstep(height * 0.20, height * 0.36, screenY);
      waterSurfaceShadowAlpha[count] = 0.30 * visibleFade * (0.52 + nearT * 0.48);
    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float outerRadius = smoothstep(0.24, 0.43, radialDist);
          float outerDirection = 0.55 + 0.45 * smoothstep(0.00, 0.42, radialOut);
          float outerWall = sideFacing * outerRadius * outerDirection;
          float lowerWall = submerged * outerWall * (0.62 + 0.38 * smoothstep(-0.08, 0.72, -n.y + 0.2));
          float exteriorWallBase = outerWall * (1.0 - smoothstep(0.38, 0.96, h));
          float exteriorWaterline = outerWall * (1.0 - smoothstep(0.54, 0.86, h));
          vec3 exteriorWallCol = vec3(0.42, 0.57, 0.62);
          vec3 exteriorLowerCol = vec3(0.12, 0.44, 0.58);
          col = mix(col, exteriorWallCol, clamp(exteriorWallBase * 0.72, 0.0, 0.78));
          col = mix(col, exteriorLowerCol, clamp(exteriorWaterline * 0.36 + lowerWall * 0.46, 0.0, 0.62));
          col += lowerWall * vec3(0.00, 0.07, 0.09) * (0.30 + specA * 0.28);
          gl_FragColor = vec4(col, 1.0);
        }
      `,
    });
