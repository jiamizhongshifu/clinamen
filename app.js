...
(() => {
    ...
    const fragmentSource = `
      ...
      void main() {
        ...
        for (int i = 0; i < 48; i += 1) {
          ...
          float mirrorY = nq.y + 0.08;
          float silhouetteWidth = 0.78 + 0.20 * smoothstep(-0.56, 0.74, mirrorY);
          float bowlShape = (nq.x / max(0.08, silhouetteWidth)) * (nq.x / max(0.08, silhouetteWidth)) + mirrorY * mirrorY * 1.18;
          float reflectionBody = 1.0 - smoothstep(0.58, 1.04, bowlShape);
          float lowerCurve = 1.0 - smoothstep(0.045, 0.18, abs((nq.x * nq.x * 0.72 + (mirrorY - 0.42) * (mirrorY - 0.42) * 3.8) - 0.54));
          float reflectionShape = max(reflectionBody * 0.72, lowerCurve * 0.34);
          reflectionShape *= smoothstep(-0.86, -0.18, mirrorY) * (1.0 - smoothstep(0.88, 1.12, mirrorY));
  ...
  function drawWaterGL(now) {
    ...
    for (const b of shadowBowls) {
      ...
      waterSurfaceShadowData[base + 2] = (b.r * (0.92 + nearT * 0.08 + farT * 0.02)) / width;
      waterSurfaceShadowData[base + 3] = (b.r * (0.24 + nearT * 0.07 - farT * 0.05)) / height;
